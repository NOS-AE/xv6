...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += szinc){
    ...
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte);
    szinc = (*pte & PTE_S) ? SUPERPGSIZE : PGSIZE;
...
#ifdef LAB_PGTBL
void
vmprint_walk(pagetable_t pagetable, int level, uint64 mask) {
  static char *level_prefix[] = {" .. .. ..", " .. ..", " .."};

  for (int i = 0; i < 512; i++) {
    pte_t pte = pagetable[i];
    if (pte & PTE_V) {
      uint64 va = mask | (uint64)(i << PXSHIFT(level));
      uint64 pa = PTE2PA(pte);
      printf("%s%p: pte %p pa %p\n", 
        level_prefix[level], (void*)(va), (void*)pte, (void*)pa);
      if(level > 0) {
